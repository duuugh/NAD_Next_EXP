...
DEFAULT_METRICS = "/home/jovyan/work/NAD_Next/result/mixed_v6_local_head_metrics.json"
...
LOCAL_FEATURES = [
    ...
    "lp1",
    "lp2",
    "lp_gap_top2_minus_top1",
    "sc_gap_top2_minus_top1",
    "len1",
    "len2",
    "len_gap_top2_minus_top1",
    "parse_ok_gap_top2_minus_top1",
    "is_int_gap_top2_minus_top1",
]


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train mixed_v6 AIME local binary corrector.")
    p.add_argument("--input", default=DEFAULT_INPUT)
    p.add_argument("--model-output", default=DEFAULT_MODEL)
    p.add_argument("--metrics-output", default=DEFAULT_METRICS)
    p.add_argument("--features", default=",".join(LOCAL_FEATURES), help="Comma-separated feature names.")
    p.add_argument("--valid-ratio", type=float, default=0.2)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--target-max-flip-rate", type=float, default=0.08)
    ...
    p.add_argument("--gap-buckets", default="", help="Optional comma-separated gap_bucket labels to keep.")
    return p.parse_args()
...
def keep_bucket(row: Dict[str, str], allowed_buckets: List[str]) -> bool:
    if not allowed_buckets:
        return True
    return row.get("gap_bucket", "") in allowed_buckets


def build_xy(rows: List[Dict[str, str]], features: List[str], allowed_buckets: List[str]) -> Tuple[np.ndarray, np.ndarray, List[str], List[str], int]:
    feats = []
    labels = []
    cache_keys = []
    problem_ids = []
    dropped_bucket = 0
    for r in rows:
        if not keep_bucket(r, allowed_buckets):
            dropped_bucket += 1
            continue
        ...
        feats.append(x)
        labels.append(to_int01(y))
        cache_keys.append(str(r.get("cache_key", "")))
        problem_ids.append(str(r.get("problem_id", "")))
    return np.asarray(feats, dtype=np.float64), np.asarray(labels, dtype=np.int64), cache_keys, problem_ids, dropped_bucket
...
def main() -> None:
    ...
    allowed_buckets = [x.strip() for x in args.gap_buckets.split(",") if x.strip()]
    x, y, cks, pids, dropped_bucket = build_xy(rows, features, allowed_buckets)
    ...
    if np.sum(valid_mask) == 0 or np.sum(train_mask) == 0:
        raise ValueError("Train/valid split failed. Adjust valid-ratio or seed.")
    ...
    metrics = {
        ...
        "features": features,
        "dropped_by_bucket": dropped_bucket,
